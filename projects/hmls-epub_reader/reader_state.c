...
enum {
    ...
    kStateVersion = 2,
    kStateVersionLegacy = 1,
    kStateHeaderSizeV1 = 20,
    kStateHeaderSize = 22
};
...
static Boolean HeaderMatchesDoc(const StateHeader* hdr, const ReaderDoc* doc) {
    if (hdr->magic != kStateMagic
        || (hdr->version != kStateVersion && hdr->version != kStateVersionLegacy)) {
        return false;
    }
...
void ReaderStateLoad(ReaderDoc* doc) {
    ...
    count = kStateHeaderSizeV1;
    err = FSRead(refNum, &count, (Ptr)&hdr);
    if (err != noErr || count != kStateHeaderSizeV1) {
        FSClose(refNum);
        return;
    }
    ...
    doc->savedLastChapter = 1;
    if (hdr.version >= kStateVersion) {
        count = 2;
        err = FSRead(refNum, &count, (Ptr)&hdr.lastChapter);
        if (err == noErr && count == 2) {
            doc->savedLastChapter = hdr.lastChapter;
            if (doc->savedLastChapter < 1) {
                doc->savedLastChapter = 1;
            }
        }
    }
...
void ReaderStateSave(ReaderDoc* doc) {
    ...
    hdr.lastChapter = doc->currentChapter;
...
void RebuildChapterMenu(const ReaderDoc* doc) {
    MenuRef menu = GetMenu(kMenuChapters);
    short count;
    short i;
    ...
    for (i = 0; i < doc->chapterCount; i++) {
        Str255 label;
        short len = doc->chapters[i].titleLen;
        short j;
        ...
        for (j = 0; j < len; j++) {
            label[j + 1] = doc->chapters[i].title[j];
        }
