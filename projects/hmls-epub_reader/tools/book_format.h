...
enum {
    ...
    kBookVersion = 4,
    kBookVersionLegacy = 3,
    kBookHeaderSize = 24,
    kBookChapterEntrySize = 10,
    kMaxChapters = 64,
    kMaxChapterTitle = 48
};
 ...
 * (Geneva 12, menu bar, nav bar, and insets match hmls-epub_reader.c).
 */
enum {
...
static inline void book_chapter_to_be(const BookChapter* in, BookChapter* out) {
    out->sourceOffset = be32(in->sourceOffset);
    out->firstPage = be32(in->firstPage);
    out->titleLen = be16(in->titleLen);
}
