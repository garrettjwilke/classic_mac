...
static int zip_seek(ZipArchive* z, long pos) {
    if (fseek(z->f, pos, SEEK_SET) != 0) return -1;
    z->pos = pos;
    return 0;
}

static int zip_read(ZipArchive* z, void* buf, long len) {
    if (fread(buf, 1, (size_t)len, z->f) != (size_t)len) return -1;
    z->pos += len;
    return 0;
}
...
ZipArchive* zip_open(const char* path) {
    ...
    if (fseek(f, tail_start, SEEK_SET) != 0) {
        fclose(f);
        epub_log("zip_open: fseek tail failed");
        return NULL;
    }
    if (fread(tail_buf, 1, (size_t)search_range, f) != (size_t)search_range) {
        fclose(f);
        epub_log("zip_open: fread tail failed");
        return NULL;
    }
    ...
    if (fseek(f, eocd_offset + 10, SEEK_SET) != 0) {
        fclose(f);
        return NULL;
    }
